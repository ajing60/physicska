...
def path (x):
    ...
    #    else:
    #        return -50
    elif path_type == "Free Fall":
        if  x_min<=x<0:
            return 0.03*(x+20)**2-40
    elif path_type == "Ramp Fancy":
        if x_min<x < -40: return 10
        if -40 <= x < -17: return (0.03*(x+40)**2) +10
        if -17<=x<0: return (-2*x)-10
        if 0<=x<50: return (-0.5*x) -10
        return -50
...
scene.append_to_caption ("\n\t")
scene.append_to_caption ("\n\t")
scene.append_to_caption("\t<b>Path Types</b>\n")

button(text = "Sine", bind=set_Sine)
button(text = "Free Fall", bind=set_free)
button(text = "Ramp Down", bind=set_down)
button(text = "Ramp Fancy", bind=set_up)
    ...
    if running: 
        ...
        if marble.pos.y <= (path (marble.pos.x)+marble_r + 0.1): #on path w some tolerance for force graph purpose
            ...
            if F_n_mag>0: #ifthere is normal force
                ...
                if(abs(v_diff) > 0.01):#if there is a significant difference 
                    ...
                    if (abs(static_max) <=k_static*F_n_mag): #less than
                        ...
                        omega=v_t/marble_r
                        F_net = F_net + F_n + F_f
                        a = F_net / (mass * (7/5)) #factor in rotational inertia 7/5
